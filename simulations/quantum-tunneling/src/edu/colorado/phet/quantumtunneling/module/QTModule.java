...
import edu.colorado.phet.quantumtunneling.view.AbstractProbabilityNode.ProbabilityDensityValueNode;
...
public class QTModule extends QTAbstractModule implements Observer {
    ...
    //private ProbabilityDensityValueNode _probabilityDensityValueNode;
    private ArrayList<ProbabilityDensityValueNode> _probabilityDensityValueNodes; // need to use diamond notation, otherwise for-each-loop throws error because of incompatible types.
    private PText _probabilityDensityLabel;
    ...
    public QTModule() {
        ...
        _probabilityDensityValueNodes = new ArrayList();
...
//            ProbabilityDensityValueNode probabilityDensityValueNode = new ProbabilityDensityValueNode(regionIndex);
//            _probabilityDensityValueNodes.add(probabilityDensityValueNode);
//            _parentNode.addChild(probabilityDensityValueNode);
//        }
//        _probabilityDensityValueNodes.add(probabilityDensityValueNode);
//        _parentNode.addChild(probabilityDensityValueNode);
        ...
        // Clock Controls
        {
            ...
            // An action listener is needed too. Best place for it would be QTControlPanel since that class is responsible for all controls.
            _recordCheckBox = new JCheckBox("Record");
            _recordCheckBox.addActionListener(listener);
            _clockControls.addToLeft(_recordCheckBox);
    ...
    public void reset() {
        ...
        // Controls
        {
            ...
            _controlPanel.setPdvSelected( false );
    ...
    public QTModuleConfig save() {
        ...
        config.setPdvSelected( _controlPanel.isPdvSelected() );
    ...
    public void load( QTModuleConfig config ) {
        ...
        _controlPanel.setPdvSelected( config.isPdvSelected() );
    ...
    private class EventListener extends ComponentAdapter implements ActionListener {
        ...
        public void actionPerformed( ActionEvent event ) {
            ...
            else if ( event.getSource() == _recordCheckBox ) {
    ...
    private void handleRecordSelection() {
        _wavePacket.setRecordEnabled(_recordCheckBox.isSelected());
    }
    ...
    public void setPotentialEnergy( AbstractPotential potentialEnergy ) {
        ...
        setPdvVisible(false);
        // Nodes have to be set invisible in order to prevent changes in wave causing calls of node's update method. Otherwise regions will be adapted in R847 and the node's update method will use wrong indices.
        ...
        recreateProbabilityDensityValueNodes();
    ...
    public void setWaveType( WaveType waveType ) {
        ...
        if ( waveType == WaveType.PLANE ) {
            ...
            for (ProbabilityDensityValueNode probabilityDensityValueNode : _probabilityDensityValueNodes) {
                probabilityDensityValueNode.setWave(_planeWave);
            }
        }
        else {
            _planeWave.setEnabled( false );
            ...
            for (ProbabilityDensityValueNode probabilityDensityValueNode : _probabilityDensityValueNodes) {
                probabilityDensityValueNode.setWave(_wavePacket);
            }
        }
    }
    ...
    private void handleClockPause() {
        setRecordCheckBoxEnabled(true);
    }

    private void handleClockStart() {
        setRecordCheckBoxEnabled(false);
    }
    ...
    public void setColorScheme( QTColorScheme colorScheme ) {
        ...
        for (ProbabilityDensityValueNode probabilityDensityValueNode : _probabilityDensityValueNodes) {
            probabilityDensityValueNode.setColorScheme(colorScheme);
        }
        _probabilityDensityLabel.setTextPaint(colorScheme.getAnnotationColor());
    }
    ...
    public void setPdvVisible(boolean selected) {
        // sets all three instances of ProbabilityDensityValueNode visible.
        for (ProbabilityDensityValueNode probabilityDensityValueNode : _probabilityDensityValueNodes) {
            probabilityDensityValueNode.setVisible(selected);
        }
        if (_probabilityDensityLabel != null) {
            _probabilityDensityLabel.setVisible(selected);
        }
    }

    public void setRecordCheckBoxEnabled(boolean enabled) {
        _recordCheckBox.setEnabled(enabled);
    }
    ...
    /**
     * Updates the layout (on the canvas) of probability density values.
     * This happens when the canvas size is changed.
     */
    private void updatePdvLayout() {
        Rectangle2D probabilityDensityPlotBounds = _chartNode.localToGlobal( _chartNode.getProbabilityDensityPlotBounds() );
        PBounds pdZoomControlBounds = _probabilityDensityZoomControl.getFullBounds();

        double x, y, standardY;
        final double xFudge = 40; // When 'P' is prepended as label, 65 is needed as value here.
        final double yFudge = 10; // Chosen through trial and error.
        final double fixedRegionWidth;

        int numberOfRegions = _potentialEnergy.getNumberOfRegions();
        fixedRegionWidth = probabilityDensityPlotBounds.getWidth()/numberOfRegions;


        standardY = probabilityDensityPlotBounds.getY() + probabilityDensityPlotBounds.getHeight()/2 - yFudge;

        double lastHandleXPosition = 0;

        for (int index = 0; index < numberOfRegions; index++) {
            ProbabilityDensityValueNode probabilityDensityValueNode = _probabilityDensityValueNodes.get(index);
            ...
            if (numberOfRegions == 1) {
                x = probabilityDensityPlotBounds.getX() + (probabilityDensityPlotBounds.getMaxX() - probabilityDensityPlotBounds.getX()) / 2;
            } else if (index == 0) {
                x = probabilityDensityPlotBounds.getX() + (_potentialEnergyControls.getRegionBoundaryDragHandle(index).getGlobalPosition().getX() - probabilityDensityPlotBounds.getX()) / 2;
                lastHandleXPosition = _potentialEnergyControls.getRegionBoundaryDragHandle(index).getGlobalPosition().getX();
            } else if (index == numberOfRegions - 1) {
                x = lastHandleXPosition + (probabilityDensityPlotBounds.getMaxX() - lastHandleXPosition) / 2;

            } else {
                x = lastHandleXPosition + (_potentialEnergyControls.getRegionBoundaryDragHandle(index).getGlobalPosition().getX() - lastHandleXPosition) / 2;
                lastHandleXPosition = _potentialEnergyControls.getRegionBoundaryDragHandle(index).getGlobalPosition().getX();
                y = y + (Math.pow(-1,index)*yFudge*2);
            }

            x = x - xFudge/2; // Was 3


            probabilityDensityValueNode.setOffset(x,y);
        }
        x = pdZoomControlBounds.getMaxX() + 15; // VARIANT SWITCH: 9
        y = probabilityDensityPlotBounds.getY() + probabilityDensityPlotBounds.getHeight()/2 - 47.5; // VARIANT SWITCH: 35
        _probabilityDensityLabel.setOffset(x,y);
    } //- - - - - - - - - - - - - - -


    /*
     * Recreates the probability density value nodes for current number of regions.
     * This happens at application start once and then each time the potential
     * energy is changed.
     */
    private void recreateProbabilityDensityValueNodes() {
        _parentNode.removeChildren(_probabilityDensityValueNodes);
        _parentNode.removeChild(_probabilityDensityLabel);
        _probabilityDensityValueNodes.clear();
        int numberOfRegions = _potentialEnergy.getNumberOfRegions();
        for (int regionIndex = 0; regionIndex < numberOfRegions; regionIndex++) {
            ProbabilityDensityValueNode probabilityDensityValueNode = new ProbabilityDensityValueNode(regionIndex);
            // Set visibility of created node according to checkbox state when changing potential energy.
            probabilityDensityValueNode.setVisible(_controlPanel.isPdvSelected());
            _probabilityDensityValueNodes.add(probabilityDensityValueNode);
            _parentNode.addChild(probabilityDensityValueNode);
            probabilityDensityValueNode.setWave(_wavePacket);
        }
        _probabilityDensityLabel = new PText(QTResources.getString("char.probabilityDensityValue") + "=");
        _probabilityDensityLabel.setFont(new PhetFont(Font.PLAIN, 18)); // VARIANT SWITCH: 60
        //_probabilityDensityLabel.setGlobalScale(5);
        _probabilityDensityLabel.setTextPaint(new Color(0, 0, 0)); // VARIANT SWITCH: additional '30'
        _probabilityDensityLabel.setVisible(_controlPanel.isPdvSelected());
        _parentNode.addChild(_probabilityDensityLabel);
        updatePdvLayout();
    }
