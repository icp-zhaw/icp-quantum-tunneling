...
public class WavePacket extends AbstractWave implements Observer, ClockListener {
    ...
    private ArrayList<Double> simulationTimes;
    private CSVWriter csvWriter;
    private boolean _recordEnabled;
    ...
    //----------------------------------------------------------------------------
    // AbstractWave override
    //----------------------------------------------------------------------------

    /**
     * Gets the probability density value.
     * A return value < 0 indicates that probability density value
     * doesn't make sense.
     *
     * @param regionIndex index of region for which to calculate probability density
     * @return double
     */
    public double getProbabilityDensityValue(int regionIndex) {
        double P = -1;
        if (_solver != null) {
            P = calculateProbabilityDensity(_pe.getStart(regionIndex), _pe.getEnd(regionIndex));
        }


        if (_recordEnabled) {
            simulationTimes.add(P);
            csvWriter.appendDouble(P);
        }
    ...
    /*
     * Calculates the probability density value for the region given by bounds a and b.
     */
    private double calculateProbabilityDensity(double a, double b) {
        double P = -1;
        double[] positions = _solver.getPositionValues();
        LightweightComplex[] energies = _solver.getWaveFunctionValues();
        ...
        if (positions.length > 1) {
            ...
            double dx = positions[1] - positions[0];
            for (int i = 0; i < positions.length; i++) {
                if (positions[i] >= a && positions[i] <= b) {
                    double abs = energies[i].getAbs();
                    P = P + abs * abs * dx;
                }
            }
    ...
    private void update() {
        ...
        if (_pe != null) {
        csvWriter = new CSVWriter(_pe.getNumberOfRegions()+1);
        csvWriter.appendDouble(0.0); }
    }
    ...
    public void clockTicked( ClockEvent clockEvent ) {
        ...
        if (_recordEnabled) {
            csvWriter.appendDouble(clockEvent.getSimulationTime());
        }
    }
